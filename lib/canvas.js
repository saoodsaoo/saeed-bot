...
export function levelup(teks, level) {
    return new Promise(async (resolve, reject) => {
        ...
        const [_spawnprocess, ..._spawnargs] = [
            ...
            'convert', xtsx,
            '-font', fontTexts, '-fill', '#0F3E6A',
            '-size', '1024x784', '-pointsize', '68',
            '-interline-spacing', '-7.5', '-annotate', '+153+200', teks,
            '-font', fontLevel, '-fill', '#0A2A48',
            '-size', '1024x784', '-pointsize', '140',
            '-interline-spacing', '-1.2', '-annotate', anotations, level,
            '-append', 'jpg:-',
        ]
        const bufs = []
