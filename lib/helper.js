...
const __filename = function filename(pathURL = import.meta, rmPrefix = os.platform() !== 'win32') {
    ...
    return rmPrefix ?
        /file:\/\/\//.test(p) ? fileURLToPath(p) : p
        : /file:\/\/\//.test(p) ? p : pathToFileURL(p).href
}
...
const API = (name, path = '/', query = {}, apikeyqueryname) =>
    (name in globalThis.APIs ? globalThis.APIs[name] : name) + path +
    (query || apikeyqueryname ? '?' + new URLSearchParams(Object.entries({
        ...query,
        ...(apikeyqueryname ? { [apikeyqueryname]: globalThis.APIKeys[name in globalThis.APIs ? globalThis.APIs[name] : name] } : {})
    })) : '')
...
const saveStreamToFile = (stream, file) => new Promise((resolve, reject) => {
    ...
    writable.once('finish', () => { resolve(); writable.destroy() })
    writable.once('error', () => { reject(); writable.destroy() })
})
...
export default {
    __filename, __dirname, __require,
    checkFileExists, API,
    saveStreamToFile, isReadableStream,
    opts, prefix,
}
